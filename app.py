...
def create_equipo():
    if request.method == 'POST':
        ...
        db.session.commit()  # <-- commit por acción CREATE
...
def edit_equipo(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()  # <-- commit por acción UPDATE
        flash(f'Equipo "{equipo.nombre}" actualizado.', 'success')
        return redirect(url_for('show_equipo', id=equipo.id))
...
def delete_equipo(id):
    ...
    db.session.commit()  # <-- commit por acción DELETE
